...
深海聲納——單人本地工具：YouTube 下載 + MLX Whisper 轉錄（Apple Silicon Metal 加速）+ 說話者識別 + AI 降噪 + TXT/SRT/DOCX 匯出。無帳號、無資料庫、無 Docker。完整實作說明見 [ARCHITECTURE.md](ARCHITECTURE.md)。
...
- **HF_TOKEN** 在 `backend/.env`（gitignore），pyannote gated model 用；帳號需在 HF 網站接受 `pyannote/speaker-diarization-community-1` 條款
