...
| 說話者識別 | pyannote.audio 4.x | MPS（Apple GPU）運算，子行程執行 |
...
│   ├── models--mlx-community--whisper-*               Whisper 模型（用過哪個才有哪個，74MB–3.1GB）
...
- 首次使用某模型時自動下載到 `models/hub/`，之後離線可用
- 「模型」頁可刪除個別 Whisper 模型；刪掉整個 `models/` 也行，下次使用會自動重新下載
- clone 專案後不需手動下載模型（除了 pyannote 需 HF token，見 §5）

### 3.2 Whisper 模型

清單定義在 `config.WHISPER_MODELS`，前端由 `GET /api/models` 取得（含本機狀態）。

| 模型 | HF repo | 下載大小 | 特性 |
|---|---|---|---|
| tiny | `mlx-community/whisper-tiny-mlx` | 74MB | 最快，準確度最低 |
| base | `mlx-community/whisper-base-mlx` | 144MB | 很快 |
| small | `mlx-community/whisper-small-mlx` | 481MB | 快，快速預覽 |
| medium | `mlx-community/whisper-medium-mlx` | 1.5GB | 較準 |
| large-v2 | `mlx-community/whisper-large-v2-mlx` | 3.1GB | 舊版最大模型 |
| large-v3 | `mlx-community/whisper-large-v3-mlx` | 3.1GB | 最準、最慢 |
| large-v3-turbo | `mlx-community/whisper-large-v3-turbo` | 1.6GB | 準又快，預設 |

注意 HF 上的實際 repo 名稱：只有 turbo 沒有 `-mlx` 後綴。舊版任務的轉錄模式對應：獵豹 = small、海豚 = large-v3-turbo、鯨魚 = large-v3。

**下載與管理（`services/models.py`）**：轉錄任務第一次用到某模型時，先進 `fetch_model` 階段下載（佔進度 30%），模型頁也能預先下載。兩者共用同一條下載執行緒；進度來自 `snapshot_download` 的彙總位元組進度條。Xet 協定已關閉（`HF_HUB_DISABLE_XET=1`），因為 Xet 只在檔案快完成時才回報進度，也無法取消；HTTP 下載可以逐塊回報、中止、續傳。下載完成後，transcriber 拿到的是本機 snapshot 路徑，mlx_whisper 載入時不會再連網。下載中的模型、或有轉錄任務正在使用的模型，都不能刪除。

## 4. 轉譯流程（`services/pipeline.py` → `transcriber.py`）

```
media 檔（任何格式）
  │ ffmpeg pipe 解碼（直接輸出 f32le，np.frombuffer 零拷貝，峰值 4 bytes/樣本）
  ▼
16kHz 單聲道 float32 波形（audio.py）（開說話者識別時，子行程同時對原始檔跑 pyannote，見 §5）
  │ silero-vad 找語音區間（max_speech_duration_s 讓過長連續語音在段內最長停頓處拆開）
  ▼
語音段（前後 pad 0.25s）
  │ ★ 音樂 fallback：語音覆蓋率 <20%（唱歌/純音樂 VAD 會失效）
  │   → 放棄 VAD，整段音檔在每 120 秒前最安靜處切塊
  ▼
合併為 ≤120 秒的塊（只在語音段間的空隙切，不切斷字；間隔 >30 秒另起一塊）
  │ 逐塊丟 mlx_whisper.transcribe（Metal GPU）
  │   - word_timestamps=True（逐字時間戳，也是說話者逐字對齊的依據）
  │   - 不用 initial_prompt（跨塊）與 hallucination_silence_threshold：實測前者慢 2.6 倍、
  │     句子黏成 30 秒一段並傳染錯字，後者會整句漏掉真實語音
  │   - MLX 快取上限 256MB（預設無上限，large-v3 50 秒內衝破 11GB）
  │   - 語言：auto 時第一塊偵測後鎖定，避免逐塊漂移
  │   - 每塊完成回報進度（真實進度 = 已處理秒數 / 語音總秒數）
  ▼
時間戳映射回原始時間軸（塊偏移量相加）
  │ 濾掉字幕署名幻覺句（Amara.org、明鏡與點點…）；連續 ≥3 句相同只留一句（音樂 fallback 不做）
...
- **MPS（Apple GPU）**：舊版 pyannote 在 MPS 有不相容運算所以曾強制 CPU；實測 pyannote 4.0.7 + torch 2.12 在 MPS 上 87 個說話時段與 CPU 逐段相同，6 分鐘音訊 289 秒 → 22 秒。失敗時自動退回 CPU 重跑。`embedding_batch_size` 32 → 8：只慢約一成，峰值 6.1GB → 3.5GB
- **spawn 子行程、與 Whisper 並行**（`Diarization`）：任務開始即啟動，父行程只輪詢共享 `Value` 的進度與 pipe 的結果；取消直接 terminate；結束後 torch/pyannote/MPS 記憶體隨行程歸還（MPS 的圖/kernel 快取約 1.6GB 在行程內釋放不掉）。不在主行程跑還有兩個原因：pyannote 相依的 optuna 會把 ImportError 的 traceback 永久存在模組裡，經 `f_back` 釘住 import 當下整條呼叫鏈的區域變數；`silero_vad` import 時把 torch 全域執行緒設為 1，舊版因此讓 pyannote 一直單執行緒在跑
- 流程：子行程自行解碼原始檔成 16k 波形 → pipeline 推理（hook 回報 segmentation/embeddings 批次進度；可指定 `num_speakers`）→ `exclusive_speaker_diarization`（pyannote 4 為對齊轉錄設計，無重疊語音）
- 說話者按首次出現順序改名為 S1/S2…（比 SPEAKER_00 好讀）
- 指派（`assign_speakers`）：**逐字**取時間重疊最長的說話者（沒重疊時取 1 秒內最近者）；句中換人且新說話者持續 ≥0.5 秒就切句，切開的句子由字詞重組文字（中文再過 s2twp）
...
- 前端 `AppContext` 每 2 秒輪詢 `/api/jobs?active=true`（閒置時 10 秒、分頁隱藏時暫停），任務完成時遞增 `jobsVersion` 通知各頁刷新
...
| GET | `/api/jobs/{id}/segments` | 轉錄結果 JSON（不含逐字時間戳 words） |
| GET | `/api/jobs/{id}/transcript?format=txt\|srt\|docx&timestamps=` | 匯出 |
| GET | `/api/models` | Whisper 模型清單＋本機狀態（已下載／下載中、佔用、是否使用中） |
| POST | `/api/models/{key}/download` | 背景預先下載 |
| POST | `/api/models/{key}/cancel` | 取消下載（有任務在等時 409） |
| DELETE | `/api/models/{key}` | 刪除本機模型（下載中或使用中時 409） |
...
- 說話者識別子行程啟動（import torch/pyannote＋載模型）約需數秒，與 Whisper 並行故通常看不出來
- large-v2／large-v3 在 16GB 機型與其他大型程式並用時可能有記憶體壓力
- 任務終止是協作式：最長要等目前推理塊（≤120 秒音訊）跑完才停
- 轉錄任務一次只跑一個（`pipeline._job_lock`），其餘顯示「排隊中」；GPU 本來就是瓶頸，並行只會疊加記憶體
